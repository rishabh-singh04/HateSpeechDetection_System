...
#                     "username": username,
#                     "email": email,
#                     "full_name": full_name if full_name else None,  # Send None if empty
#                     "password": password
#                 }
#             )
...
#                         # Handle validation errors that return multiple fields
    ...
    with st.form("login_form"):
        ...
        if submitted:
            ...
            try:
                ...
                if response and response.status_code == 200:
                    st.session_state.access_token = response.json().get("access_token")
                    st.session_state.username = username_or_email  # Store username
                    st.success("Logged in successfully!")
                    st.rerun()
                elif response:
                    st.error(f"Login failed: {response.json().get('detail', 'Unknown error')}")
